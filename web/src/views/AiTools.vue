...
async function load() {
  try {
    ;[jds.value, resumes.value] = await Promise.all([
...
async function importJd() {
  try {
    ...
    const r = await api.post('/api/ai/import-result', { type: 'jd_parse', jd_id: Number(jdSel.value), result })
    toast('success', 'JD 解析结果已导入')
  } catch (e: any) {
    toast('error', e.message || 'JSON 解析失败')
  }
}
...
async function importMatch() {
  try {
    const result = {
      jd_id: Number(mJd.value),
      resume_id: Number(mResume.value),
...
<template>
  <div class="grid grid-cols-1 gap-5 lg:grid-cols-3">
    ...
    <section class="card space-y-3">
      ...
      <button class="btn-primary w-full" @click="exportJd">导出 prompt</button>
      <textarea v-model="jdPrompt" rows="6" readonly class="input bg-slate-50" placeholder="导出的 prompt 将显示在此"></textarea>
      <button v-if="jdPrompt" class="btn-ghost w-full" @click="copy(jdPrompt)">复制 prompt</button>
      <div class="border-t border-slate-100 pt-3">
        <label class="label">粘贴 AI 返回 JSON 后导入</label>
        <textarea v-model="jdImportJson" rows="4" class="input" placeholder='{"responsibilities":[],"requirements":[],"keywords":[]}'></textarea>
    ...
    <section class="card space-y-3">
      ...
      <div class="flex gap-2">
        <button class="btn-primary flex-1" @click="exportMatch">导出 prompt</button>
        <button class="btn-ghost flex-1" @click="matchLocal">本地即时匹配</button>
      </div>
      <textarea v-model="mPrompt" rows="4" readonly class="input bg-slate-50" placeholder="导出的 prompt 将显示在此"></textarea>
      ...
      <div v-if="matchRes" class="rounded-lg bg-slate-50 p-3 text-sm">
        ...
        <div class="mt-2 text-slate-600">
          缺失项：{{ (matchRes.missing_points || []).join('、') || '无' }}
        </div>
    ...
    <section class="card space-y-3">
      ...
      <button class="btn-primary w-full" @click="exportQ">导出 prompt</button>
      <textarea v-model="qPrompt" rows="6" readonly class="input bg-slate-50" placeholder="导出的 prompt 将显示在此"></textarea>
      <button v-if="qPrompt" class="btn-ghost w-full" @click="copy(qPrompt)">复制 prompt</button>
      <div class="border-t border-slate-100 pt-3">
        <label class="label">粘贴 AI 返回 JSON 后导入</label>
        <textarea v-model="qImportJson" rows="4" class="input" placeholder='{"questions":[{"question":"...","category":"..."}]}'></textarea>
